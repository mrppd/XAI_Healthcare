...
X_train, X_test, y_train, y_test = train_test_split(df_all_new_sel.drop(['Entscheidung'], axis=1), 
                                                    df_all_new_sel.Entscheidung, test_size=0.2, random_state=random_state)
...
merged_df_FI_normal = merge_dicts_to_dataframe(feature_importances_dt, feature_importances_sh)
merged_df_FI_normal.to_csv('XAI/merged_df_FI_normal.csv', index=True)
print(merged_df_FI_normal)
